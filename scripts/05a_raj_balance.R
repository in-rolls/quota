...
raj_elex_vd_01 <- raj_elex_vd_01 %>%
     mutate(treat = paste(female_res_2005, female_res_2010, sep = "-"),
            treat = recode(treat,
                           ...
                           `1-0` = "F-NF",
                           `1-1` = "F-F"),
            cluster_id = paste(dist_name_new_2005, samiti_name_new_2005, gp_new_2005, sep = "-"))  

# GP Level
raj_elex_vd_01_gp <- raj_elex_vd_01 %>%
...
f_stats <- map_dbl(names(balance_vars), ~ calculate_ri_p_value(.x, raj_elex_vd_01_gp))
